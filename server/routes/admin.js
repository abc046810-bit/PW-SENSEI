...
async function handleSaveUnifiedLecture(req, res, targetChapterId, existingLectureId = null) {
  ...
  const lecture_date = b.lecture_date || '';
  const is_today = b.is_today || 0;
  ...
  try {
    ...
    const finalDate = normalizeDate(lecture_date);
...
// TEACHERS
router.get('/teachers', isAdmin, async (req, res) => {
  try { const teachers = await db.all('SELECT * FROM teachers ORDER BY display_order ASC, id ASC'); res.json({ teachers }); }
  catch (err) { res.status(500).json({ error: 'Failed to fetch teachers', teachers: [] }); }
});

router.post('/teachers', isAdmin, async (req, res) => {
  const { name, subject, bio = '', photo_url = '', batch_id = null, display_order = 0, is_active = 1 } = req.body;
  if (!name || !name.trim()) return res.status(400).json({ error: 'Teacher name is required' });
  try {
    const result = await db.run('INSERT INTO teachers (name, subject, bio, photo_url, batch_id, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [name.trim(), subject ? String(subject).trim() : '', bio ? String(bio).trim() : '', photo_url ? String(photo_url).trim() : '', batch_id ? parseInt(batch_id, 10) : null, parseInt(display_order, 10) || 0, is_active ? 1 : 0]);
    const teacher = await db.get('SELECT * FROM teachers WHERE id = ?', [result.lastInsertRowid]);
    if (!teacher) return res.status(500).json({ error: 'Failed to verify created teacher' });
    res.status(201).json({ success: true, teacher });
  } catch (err) { console.error('Add teacher error:', err); res.status(500).json({ error: 'Failed to add teacher: ' + (err.message || 'Server error') }); }
});

router.put('/teachers/:id', isAdmin, async (req, res) => {
  const id = parseInt(req.params.id, 10);
  const { name, subject, bio, photo_url, batch_id, display_order, is_active } = req.body;
  if (!name || !name.trim()) return res.status(400).json({ error: 'Teacher name is required' });
  try {
    const existing = await db.get('SELECT id FROM teachers WHERE id = ?', [id]);
    if (!existing) return res.status(404).json({ error: 'Teacher not found' });
    await db.run('UPDATE teachers SET name = ?, subject = ?, bio = ?, photo_url = ?, batch_id = ?, display_order = ?, is_active = ? WHERE id = ?',
      [name.trim(), subject ? String(subject).trim() : '', bio ? String(bio).trim() : '', photo_url ? String(photo_url).trim() : '', batch_id ? parseInt(batch_id, 10) : null, parseInt(display_order, 10) || 0, is_active !== undefined ? (is_active ? 1 : 0) : 1, id]);
    const teacher = await db.get('SELECT * FROM teachers WHERE id = ?', [id]);
    if (!teacher) return res.status(500).json({ error: 'Failed to verify updated teacher' });
    res.json({ success: true, teacher });
  } catch (err) { console.error('Edit teacher error:', err); res.status(500).json({ error: 'Failed to update teacher: ' + (err.message || 'Server error') }); }
});
...
router.delete('/teachers/:id', isAdmin, async (req, res) => {
  ...
  try {
    const existing = await db.get('SELECT id, name FROM teachers WHERE id = ?', [id]);
    if (!existing) return res.status(404).json({ error: 'Teacher not found or already deleted' });
    await db.run('DELETE FROM teachers WHERE id = ?', [id]);
    const check = await db.get('SELECT id FROM teachers WHERE id = ?', [id]);
    if (check) return res.status(500).json({ error: 'Failed to delete teacher: record still exists' });
    res.json({ success: true, message: 'Teacher deleted' });
  } catch (err) { console.error('Delete teacher error:', err); res.status(500).json({ error: 'Failed to delete teacher: ' + (err.message || 'Server error') }); }
});

// NAV LINKS
router.get('/nav-links', isAdmin, async (req, res) => {
  try { const navLinks = await db.all('SELECT * FROM nav_links ORDER BY display_order ASC, id ASC'); res.json({ navLinks }); }
  catch (err) { res.status(500).json({ error: 'Failed to fetch nav links', navLinks: [] }); }
});

router.post('/nav-links', isAdmin, async (req, res) => {
  const { label, url, icon = '', display_order = 0, is_active = 1 } = req.body;
  if (!label || !label.trim() || !url || !url.trim()) return res.status(400).json({ error: 'Label and URL are required' });
  try {
    const result = await db.run('INSERT INTO nav_links (label, url, icon, display_order, is_active) VALUES (?, ?, ?, ?, ?)',
      [label.trim(), url.trim(), icon ? String(icon).trim() : '', parseInt(display_order, 10) || 0, is_active ? 1 : 0]);
    const navLink = await db.get('SELECT * FROM nav_links WHERE id = ?', [result.lastInsertRowid]);
    if (!navLink) return res.status(500).json({ error: 'Failed to verify created nav link' });
    res.status(201).json({ success: true, navLink });
  } catch (err) { console.error('Add nav link error:', err); res.status(500).json({ error: 'Failed to add nav link: ' + (err.message || 'Server error') }); }
});

router.put('/nav-links/:id', isAdmin, async (req, res) => {
  const id = parseInt(req.params.id, 10);
  const { label, url, icon, display_order, is_active } = req.body;
  if (!label || !label.trim() || !url || !url.trim()) return res.status(400).json({ error: 'Label and URL are required' });
  try {
    const existing = await db.get('SELECT id FROM nav_links WHERE id = ?', [id]);
    if (!existing) return res.status(404).json({ error: 'Nav link not found' });
    await db.run('UPDATE nav_links SET label = ?, url = ?, icon = ?, display_order = ?, is_active = ? WHERE id = ?',
      [label.trim(), url.trim(), icon ? String(icon).trim() : '', parseInt(display_order, 10) || 0, is_active !== undefined ? (is_active ? 1 : 0) : 1, id]);
    const navLink = await db.get('SELECT * FROM nav_links WHERE id = ?', [id]);
    if (!navLink) return res.status(500).json({ error: 'Failed to verify updated nav link' });
    res.json({ success: true, navLink });
  } catch (err) { console.error('Edit nav link error:', err); res.status(500).json({ error: 'Failed to update nav link: ' + (err.message || 'Server error') }); }
});
...
router.delete('/nav-links/:id', isAdmin, async (req, res) => {
  ...
  try {
    const existing = await db.get('SELECT id, label FROM nav_links WHERE id = ?', [id]);
    if (!existing) return res.status(404).json({ error: 'Nav link not found or already deleted' });
    await db.run('DELETE FROM nav_links WHERE id = ?', [id]);
    const check = await db.get('SELECT id FROM nav_links WHERE id = ?', [id]);
    if (check) return res.status(500).json({ error: 'Failed to delete nav link: record still exists' });
    res.json({ success: true, message: 'Nav link deleted' });
  } catch (err) { console.error('Delete nav link error:', err); res.status(500).json({ error: 'Failed to delete nav link: ' + (err.message || 'Server error') }); }
});

// BANNERS
router.get('/banners', isAdmin, async (req, res) => {
  try { const banners = await db.all('SELECT * FROM banners ORDER BY display_order ASC, id ASC'); res.json({ banners }); }
  catch (err) { res.status(500).json({ error: 'Failed to fetch banners', banners: [] }); }
});

router.post('/banners', isAdmin, async (req, res) => {
  const { image_url, title = '', subtitle = '', link_url = '', display_order = 0, is_active = 1 } = req.body;
  if (!image_url || !image_url.trim()) return res.status(400).json({ error: 'Banner image URL is required' });
  try {
    const result = await db.run('INSERT INTO banners (image_url, title, subtitle, link_url, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?)',
      [image_url.trim(), title ? String(title).trim() : '', subtitle ? String(subtitle).trim() : '', link_url ? String(link_url).trim() : '', parseInt(display_order, 10) || 0, is_active ? 1 : 0]);
    const banner = await db.get('SELECT * FROM banners WHERE id = ?', [result.lastInsertRowid]);
    if (!banner) return res.status(500).json({ error: 'Failed to verify created banner' });
    res.status(201).json({ success: true, banner });
  } catch (err) { console.error('Add banner error:', err); res.status(500).json({ error: 'Failed to add banner: ' + (err.message || 'Server error') }); }
});
...
router.put('/banners/:id', isAdmin, async (req, res) => {
  ...
  try {
    ...
    await db.run('UPDATE banners SET image_url = ?, title = ?, subtitle = ?, link_url = ?, display_order = ?, is_active = ? WHERE id = ?',
      [image_url.trim(), title ? String(title).trim() : '', subtitle ? String(subtitle).trim() : '', link_url ? String(link_url).trim() : '', parseInt(display_order, 10) || 0, is_active !== undefined ? (is_active ? 1 : 0) : 1, id]);
    const banner = await db.get('SELECT * FROM banners WHERE id = ?', [id]);
    if (!banner) return res.status(500).json({ error: 'Failed to verify updated banner' });
    res.json({ success: true, banner });
  } catch (err) { console.error('Edit banner error:', err); res.status(500).json({ error: 'Failed to update banner: ' + (err.message || 'Server error') }); }
});

router.delete('/banners/:id', isAdmin, async (req, res) => {
  const id = parseInt(req.params.id, 10);
  try {
    const existing = await db.get('SELECT id, title FROM banners WHERE id = ?', [id]);
    if (!existing) return res.status(404).json({ error: 'Banner not found or already deleted' });
    await db.run('DELETE FROM banners WHERE id = ?', [id]);
    const check = await db.get('SELECT id FROM banners WHERE id = ?', [id]);
    if (check) return res.status(500).json({ error: 'Failed to delete banner: record still exists' });
    res.json({ success: true, message: 'Banner deleted' });
  } catch (err) { console.error('Delete banner error:', err); res.status(500).json({ error: 'Failed to delete banner: ' + (err.message || 'Server error') }); }
});
...
router.get('/settings', isAdmin, async (req, res) => {
  try {
    ...
    res.json({ settings });
  } catch (err) { console.error('Settings error:', err); res.status(500).json({ error: 'Failed to fetch settings' }); }
});
...
router.put('/settings', isAdmin, async (req, res) => {
  ...
  try {
    for (const [key, value] of Object.entries(settings)) {
      ...
      if (existing) { await db.run('UPDATE site_settings SET value = ? WHERE key = ?', [String(value), key]); }
      else { await db.run('INSERT INTO site_settings (key, value) VALUES (?, ?)', [key, String(value)]); }
    }
